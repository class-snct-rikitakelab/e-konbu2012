...
void RA_linetrace_PID(int forward_speed,int resetflag);
...
void shock(void){
	...
	if((ecrobot_get_gyro_sensor(NXT_PORT_S4) <= gyro_offset-PM_GYRO || 
		ecrobot_get_gyro_sensor(NXT_PORT_S4) >= gyro_offset+PM_GYRO) &&
...
void RN_setting()
{
	switch (setting_mode){
		...
		case(RN_SLOW_RUN):
			...
			if(distance_now - distance_before >= 10)
			{
				//ecrobot_set_motor_rev(NXT_PORT_B,0);
				//ecrobot_set_motor_rev(NXT_PORT_C,0);
