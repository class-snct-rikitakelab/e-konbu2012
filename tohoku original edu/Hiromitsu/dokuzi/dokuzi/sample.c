...
void self_loc(){
	float v,v_r,v_l;
	float theta;
	float theta_r,theta_r_prev,delta_theta_r; 
	float theta_l,theta_l_prev,delta_theta_l; 
	
	float omega,omega_r,omega_l;
	static float theta_robo=0,x_r=0,x_y =0;

	float theta_robo_init=0,x_r_init=0,y_r_init=0;

	
	float wheel_hankei = 4.1; //[cm]
	float W = 16.2 ;; //[cm]
	static int self_loc_counter = 0;
	++self_loc_counter;


	//　右車輪の移動速度の計算始まり
	theta_r = nxt_motor_get_count(NXT_PORT_B) *(PI/180);
	
	delta_theta_r = theta_r - theta_r_prev ; //今の回転量　- 一個前の車輪の回転量
	
	theta_r_prev = theta_r; //一回前の回転量の保存
	omega_r = delta_theta_r / 0.004 ; // 4ms ごとに呼び出すので4msで割っている。

	v_r = wheel_hankei * omega_r;

	//左車輪の移動速度の計算終わり
	
	//　右車輪の移動速度の計算始まり
	theta_l = nxt_motor_get_count(NXT_PORT_B) *(PI/180);
	
	delta_theta_l = theta_l - theta_l_prev ; //今の回転量　- 一個前の車輪の回転量
	theta_l_prev = theta_l; //一回前の回転量の保存
	omega_l = delta_theta_l / 0.004 ; // 4ms ごとに呼び出すので4msで割っている。

	v_l = wheel_hankei * omega_l;
	...
	theta_robo +=theta * 0.004;
	x_r+= v*cos(theta_robo) +0.004 ;
