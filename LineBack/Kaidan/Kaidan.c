...
//PID制御ライントレース関数
float RA_linetrace_PID(int forward_speed) {
	...
	controlVals.turn_val = -(Kp * hensa + Ki * i_hensa + Kd * d_hensa);
...
void RN_setting()
{
	...
	switch (setting_mode){
		...
		//if(GYRO_OFFSET - 100 > ecrobot_get_gyro_sensor(NXT_PORT_S1) || GYRO_OFFSET + 100 < ecrobot_get_gyro_sensor(NXT_PORT_S1))
				if(LineBack_debugLineBackSignalReceive(&mLineBack) == TRUE)
			{
...
TASK(LogTask)
{
	logSend(velocity,shock(STEP_BATTERY),theta*(180/M_PI)/*distance_gyro_up - distance_before_step*/,ecrobot_get_gyro_sensor(NXT_PORT_S1),
