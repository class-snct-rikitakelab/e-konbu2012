...
static unsigned int SEESAW_BLACK_VALUE;	//黒値
static unsigned int SEESAW_WHITE_VALUE;	//白値
static unsigned int SEESAW_GRAY_VALUE;		//灰色値（現在は黒と白の平均値）
	...
	switch (setting_mode){
		...
		case (RN_RUN):
			...
			if(RN_rapid_speed_up_signal_recevie() == 1)
			{
				...
				distance_before_seesaw = fabs(CIRCUMFERENCE/360.0 * ((revL+revR)/2.0));	//段差突入時の距離を測定
			}
