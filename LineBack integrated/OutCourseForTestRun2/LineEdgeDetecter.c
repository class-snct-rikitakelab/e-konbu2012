...
int LineEdgeDetecter_detectWhite(LineEdgeDetecter *this_LineEdgeDetecter){
	...
	if((float)ecrobot_get_light_sensor(NXT_PORT_S3)  < WHITE_VALUE + LINE_THRESHOLD  
	&& (float)ecrobot_get_light_sensor(NXT_PORT_S3)  > WHITE_VALUE - LINE_THRESHOLD ){
		result=1;
	}
	return result;
}
...
int LineEdgeDetecter_detectBlack(LineEdgeDetecter *this_LineEdgeDetecter){
	int result=0;
	if((float)ecrobot_get_light_sensor(NXT_PORT_S3)  < BLACK_VALUE + LINE_THRESHOLD  
	&& (float)ecrobot_get_light_sensor(NXT_PORT_S3)  >  BLACK_VALUE - LINE_THRESHOLD ){
		result=1;
	}
	return result;
}
...
int LineEdgeDetecter_detectLineEdge(LineEdgeDetecter *this_LineEdgeDetecter){
	...
	if((float)ecrobot_get_light_sensor(NXT_PORT_S3)  <GRAY_VALUE + LINE_THRESHOLD  
	&& (float)ecrobot_get_light_sensor(NXT_PORT_S3)  > GRAY_VALUE - LINE_THRESHOLD ){
		result=1;
		ecrobot_sound_tone(880, 512, 50);
	}
return result;
