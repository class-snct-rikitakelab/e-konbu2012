...
void RN_setting(){
	...
	case (RN_RUN):
		...
		//cmd_turn    = RA_linetrace_PID() + RA_curvatureCtrl_PID(trgt_R);
		wait_count++;
		break;
...
S8 markerDetector(){
	//輝度値の偏差の偏差の算出
	static S16 LV_buf = 0;
	static S16 dev_buf = 0;

	S16 LV   = ecrobot_get_light_sensor(NXT_PORT_S3);
	S16 dev  = LV - LV_buf;
	S16 dev2 = dev - dev_buf;

	LV_buf  = LV;
	dev_buf = dev;

	//マーカー検知アルゴリズム ver.2
	static S8 dev2_buf[3] = {0, 0, 0};
	dev2_buf[2] = dev2_buf[1];
	dev2_buf[1] = dev2_buf[0];
	dev2_buf[0] = dev2;
	
	if(dev2_buf[2] >= 5 && dev2_buf[0] <= 15){
		return 1;
	}
	return 0;
}
